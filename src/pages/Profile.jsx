...
function ProfileView({ profile, avatarUrl, user, onEdit }) {
  function tryParse(val, fallback) {
    try { return JSON.parse(val) || fallback; } catch { return fallback; }
  }
  const schools = tryParse(profile.schools, []);
  const favLessons = tryParse(profile.favorite_lessons, []);
  const hobbies = tryParse(profile.hobbies, []);

  return (
    <div className="profile-view">
      <div className="profile-avatar-wrap">
        <img src={avatarUrl || DEFAULT_AVATAR} alt="avatar" className="profile-avatar" style={{ cursor: 'default' }} />
      </div>
      ...
      <div className="pv-card">
        <div className="pv-section">📋 Personal Info</div>
        <div className="pv-row"><span className="pv-label">📞 Phone</span><span>{profile.phone || <em className="pv-empty">Not set</em>}</span></div>
        <div className="pv-row"><span className="pv-label">🏠 Address</span><span>{profile.home_address || <em className="pv-empty">Not set</em>}</span></div>
        <div className="pv-row">
          <span className="pv-label">🏫 Schools</span>
          <span>{schools.length ? schools.map((s,i) => <span key={i} className="tag" style={{ marginRight: 4 }}>{s}</span>) : <em className="pv-empty">Not set</em>}</span>
        </div>
      </div>

      <div className="pv-card">
        <div className="pv-section">✨ About Me</div>
        {profile.dreams && <div className="pv-block"><div className="pv-label">🌟 My Dreams</div><p>{profile.dreams}</p></div>}
        {favLessons.length > 0 && (
          <div className="pv-block">
            <div className="pv-label">📚 Favorite Lessons</div>
            <div className="tags">{favLessons.map((l,i) => <span key={i} className="tag">{l}</span>)}</div>
          </div>
        )}
        {hobbies.length > 0 && (
          <div className="pv-block">
            <div className="pv-label">🎯 Hobbies</div>
            <div className="tags">{hobbies.map((h,i) => <span key={i} className="tag">{h}</span>)}</div>
          </div>
        )}
        {profile.fears && <div className="pv-block"><div className="pv-label">😰 What I Fear</div><p>{profile.fears}</p></div>}
      </div>

      <button className="btn btn-primary btn-full" onClick={onEdit} style={{ marginTop: 8 }}>
        ✏️ Edit Profile
      </button>
    </div>
  );
...
export default function Profile() {
  ...
  const loadProfile = () => {
    api.get('/profile/me', token).then(p => {
      ...
      // Show view mode if profile has any data filled
      const hasData = p.phone || p.dreams || p.home_address;
      setEditing(!hasData);
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      setProfile(p => ({ ...p, avatar_path: res.avatar_path }));
    } catch (err) {
      setMsg(err.message);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await api.get('/profile/me', token).then(p => setProfile(p));
      setEditing(false);
    } catch (err) {
      setMsg(err.message);
    } finally {
  ...
  return (
    <div className="profile-page">
      <div className="profile-header">
        ...
        {!editing && (
          <button className="btn btn-secondary btn-sm" style={{ marginLeft: 'auto' }} onClick={() => fileRef.current?.click()}>
            📷 Change Photo
          </button>
        )}
      ...
      {profile && !editing ? (
        <ProfileView profile={profile} avatarUrl={avatarUrl} user={user} onEdit={() => setEditing(true)} />
      ) : (
        <form className="profile-form" onSubmit={handleSave}>
          ...
          <TagInput label="🏫 Schools (add all your schools)" values={schools} onChange={setSchools} placeholder="Type school name and press Enter or Add" />
          ...
          <TagInput label="📚 Favorite Lessons" values={favLessons} onChange={setFavLessons} placeholder="e.g. Mathematics" />
          <TagInput label="🎯 Hobbies (at least 2)" values={hobbies} onChange={setHobbies} placeholder="e.g. Reading, Football..." />
          ...
          {msg && <div className={`profile-msg ${msg.includes('saved') || msg.includes('updated') ? 'success' : 'error'}`}>{msg}</div>}

          <div style={{ display: 'flex', gap: 10 }}>
            {profile && <button type="button" className="btn btn-outline" onClick={() => setEditing(false)}>Cancel</button>}
