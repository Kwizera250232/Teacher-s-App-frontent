...
export default function ClassmateProfileModal({ person, onClose, onMessage }) {
  const { token } = useAuth();
  const [profileData, setProfileData] = useState(person.i_subscribed !== undefined ? person : null);
  const [loading, setLoading] = useState(person.i_subscribed === undefined);
  ...
  useEffect(() => {
    if (person.i_subscribed !== undefined) return;
    setLoading(true);
