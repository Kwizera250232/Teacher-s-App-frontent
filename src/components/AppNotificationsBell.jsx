...
export default function AppNotificationsBell({ className = '', basePath }) {
  ...
  const {
    ...
    openNotification,
    supported,
  } = useAppNotifications(token, user?.role, { basePath });
  ...
  if (!token || !user?.role || !SUPPORTED_ROLES.has(user.role) || !supported) return null;

  const emptyHint = user.role === 'parent'
    ? 'You\'ll see when your child submits quizzes, homework, and class updates.'
    : user.role === 'student'
      ? 'You\'ll see class updates here.'
      : 'You\'ll see when students submit quizzes, homework, and team reports.';
          ...
          <div ref={panelRef} className="student-notif-panel" role="dialog" aria-label="Notifications">
            ...
            <div className="student-notif-list">
              {loading && !notifications.length ? (
                ...
                <p className="student-notif-empty">{emptyHint}</p>
              )}
            </div>
          </div>
        </>
